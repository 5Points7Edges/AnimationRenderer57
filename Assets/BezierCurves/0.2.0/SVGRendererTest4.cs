 ...
 * The triangulation of internal area cannot handle polygons that include holes.(method updated, looks better)
 * input data structure is a svg file
 * FPS (ChuShiBiao.svg): ca. 97 fps
 * The color is customizable
 */
[RequireComponent(typeof(MeshFilter), typeof(MeshRenderer))]
public class SVGRendererTest4 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = 0; i < Coordinates.Count; i++)
        {
            ...
            for (int j = 0; j < Unpacked[Coordinates[i].id].Count; j++)
            {
                ...
                shape.AddComponent<CurveDrawer4>();
                shape.GetComponent<CurveDrawer4>().material = new Material(material);
                shape.GetComponent<CurveDrawer4>().BoundingBoxComputeShader = BoundingBoxComputeShader;
                for (int k = 0; k < controlPoints.Count; k++)
                {
                    controlPoints[k] = new float3(transform.TransformPoint(new Vector3(controlPoints[k].x, controlPoints[k].y, controlPoints[k].z)));
                }
                //Debug.Log(hullPoints.Count);
                shape.GetComponent<CurveDrawer4>().controlPoints = controlPoints;
