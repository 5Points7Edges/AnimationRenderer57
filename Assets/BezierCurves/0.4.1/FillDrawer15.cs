...
public class FillDrawer15 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            SubPath15 controlPointsSubPath=pathInitial.subPaths[i];
        ...
        {
            SubPath15 controlPointsSubPath=pathEnd.subPaths[i];
    ...
    public void findtheBestOder()
    {
        ...
        {
            SubPath15 contourInitial = pathInitial.subPaths[contourIndex];
            SubPath15 contourFinal = pathEnd.subPaths[contourIndex];
            int bestStartID = 0;
            ...
            contourInitial.segments = contourInitial.segments.GetRange(bestStartID, count - bestStartID).Concat(
                contourInitial.segments.GetRange(0, bestStartID)).ToList<Segment15>();
        }
    }

    public float calculateTotalDistance(int startID,SubPath15 contourInitial,SubPath15 contourEnd)
    {
    ...
    public void insertExtraPoints()
    {
        ...
        if (difference != 0)
        {
            // the path which points should be added to
            Path15 pathLess = pathInitial.subPaths.Count() > pathEnd.subPaths.Count() ? pathEnd : pathInitial;
            // the path that has more contours
            Path15 pathMore = pathInitial.subPaths.Count() > pathEnd.subPaths.Count() ? pathInitial : pathEnd;
            
            for (int contourIndex = 0; contourIndex < difference; contourIndex++)
            {
                // pathLess.Add(new SubPath15(pathMore.subPaths[contourIndex+pathLess.subPaths.Count].getCentralPoint()));
                pathLess.Add(new SubPath15(pathLess.subPaths[0].getCentralPoint()));
            }
        }
        ...
        for (int contourIndex = 0; contourIndex < pathInitial.subPaths.Count; contourIndex++)
        {
            ...
            SubPath15 contourFinal = pathEnd.subPaths[contourIndex];
            //contourInitial always has less segments then contourFinal, so that extra points will be added to contourInitial
            ...
            while (PointsNeedToAdd > 0)
            {
                ...
                Segment15 firstCurve = new Segment15();
                ...
                Segment15 secondCurve = new Segment15();
