 ...
 * FPS (ChuShiBiao.svg): ca. 9-10 fps
 * The color is customizable
 */
[RequireComponent(typeof(MeshFilter), typeof(MeshRenderer))]
public class SVGRendererTest3 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = 0; i < Coordinates.Count; i++)
        {
            ...
            for (int j = 0; j < Unpacked[Coordinates[i].id].Count; j++)
            {
                ...
                shape.AddComponent<CurveDrawer3>();
                shape.GetComponent<CurveDrawer3>().material = material;
                shape.GetComponent<CurveDrawer3>().BoundingBoxComputeShader = BoundingBoxComputeShader;
                for (int k = 0; k < controlPoints.Count; k++)
                {
                    controlPoints[k] = new float3(transform.TransformPoint(new Vector3(controlPoints[k].x, controlPoints[k].y, controlPoints[k].z)));
                }
                shape.GetComponent<CurveDrawer3>().controlPoints = controlPoints;
