...
public class SVGRendererTest6 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int CoordinateIndex = 0; CoordinateIndex < Coordinates.Count; CoordinateIndex++)
        {
            ...
            List<GameObject> tmpShapeGameObjects = new List<GameObject>();
            for (int MIndex = 0; MIndex < UnpackedPaths[Coordinates[CoordinateIndex].pathID].Count; MIndex++)
            {
                ...
                if (orientation == 1)
                {
                    ...
                    shape.AddComponent<CurveDrawer6>();
                    shape.GetComponent<CurveDrawer6>().material = new Material(material);
                    shape.transform.parent = transform;
                    
                    shape.GetComponent<CurveDrawer6>().hullPoints = controlPointsOfaM;
            ...
            for (int MIndex = 0; MIndex < UnpackedPaths[Coordinates[CoordinateIndex].pathID].Count; MIndex++)
            {
                List<float3> controlPointsOfaM =
                    new List<float3>(UnpackedPaths[Coordinates[CoordinateIndex].pathID][MIndex]);
                foreach(GameObject addedShape in tmpShapeGameObjects)
                {
                    if (addedShape.GetComponent<CurveDrawer6>().isINPolygon(controlPointsOfaM[0]))
                    {
                        addedShape.GetComponent<CurveDrawer6>().holePoints.Add(controlPointsOfaM);
                        break;
                    }
                }
            ...
            foreach (var Gobject in tmpShapeGameObjects)
            {
                allShapeGameObjects.Add(Gobject);
            }
