 ...
 * v0.2.1
 * svg Renderer
 * This renderer can render a single shape with convex or concave cubic curve defined polygons,
 * The shape can now have holes.
 * input data structure is a svg file
 * It should be given a list of hull points and a 2D array of hole points( a list of closed hole path )
 * The color is customizable
 */
[RequireComponent(typeof(MeshFilter), typeof(MeshRenderer))]
public class SVGRendererTest5 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int CoordinateIndex = 0; CoordinateIndex < Coordinates.Count; CoordinateIndex++)
        {
            ...
            shape.AddComponent<CurveDrawer5>();
            shape.GetComponent<CurveDrawer5>().material = new Material(material);
            ...
            for (int MIndex = 0; MIndex < UnpackedPaths[Coordinates[CoordinateIndex].pathID].Count; MIndex++)
            {
                ...
                List<float3> controlPointsOfaM = new List<float3>(UnpackedPaths[Coordinates[CoordinateIndex].pathID][MIndex]);
                for (int k = 0; k < controlPointsOfaM.Count; k++)
                {
                    //controlPointsOfaM[k] = new float3(controlPointsOfaM[k].x + Coordinates[CoordinateIndex].x, -(controlPointsOfaM[k].y + Coordinates[CoordinateIndex].y), 0);
                    controlPointsOfaM[k] = new float3(controlPointsOfaM[k].x, -(controlPointsOfaM[k].y), 0);
                }
                for (int k = 0; k < controlPointsOfaM.Count; k++)
                {
                    controlPointsOfaM[k] = new float3(transform.TransformPoint(new Vector3(controlPointsOfaM[k].x, controlPointsOfaM[k].y, controlPointsOfaM[k].z)));
                }
                ...
                {
                    hasM++;
                    allShapeGameObjects.Last().GetComponent<CurveDrawer5>().hullPoints = controlPointsOfaM;
                    
                }
                else
                {
                    if (controlPointsOfaM.Count == 0) Debug.Log("-----INFO-----------------------------------------------");
                    //if(hasM!=0)
                    allShapeGameObjects.Last().GetComponent<CurveDrawer5>().holePoints.Add(controlPointsOfaM);
                }
            }
